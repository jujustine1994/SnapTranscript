...
#
# 給 GUI（進階設定視窗的「檢查更新」按鈕）呼叫，也可以獨立在終端機執行測試。
...
#   error             - 其他未預期錯誤
#
# ⚠ 這支腳本只碰「程式碼」路徑，刻意排除 config.json / .env 等使用者本機資料
#   （見下方 $CodePaths，對照 .gitignore：venv/、__pycache__/、*.pyc、.env、
#   config.json、*.log、logs/、_temp_seg_*）。
...
try {
    ...
    # 只同步「程式碼」路徑，排除 config.json（使用者設定）、.env（API Key）、
    # logs/、__pycache__、_temp_seg_* 這些本機產生物或使用者資料。
    # 2026-08-26 目錄整理：.py 與 locales/ 都搬進 src/，"src" 一項涵蓋全部。
    $CodePaths = @(
        "src", "tests", "docs", "scripts",
        "requirements.txt", "launcher.ps1"
    )
