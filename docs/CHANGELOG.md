...
- 合併輸出含段落標記的 TXT 檔案
- API Key 讀取（.env）／按「儲存」按鈕才寫入，並顯示「已儲存」提示
- 背景執行緒處理，UI 不凍結
...
### 2026-09-09 — 修正任務完成後按鈕被視窗高度切掉看不到

`_position_window()` 只在程式啟動時算一次視窗高度（依 `winfo_reqheight()`），
但「開啟資料夾」「重試失敗的 N 段」這兩顆按鈕當時還沒 `pack`（要等任務跑完才
動態顯示），reqheight 完全沒算進它們的空間。加上主視窗 `resizable(False, False)`，
高度就此鎖死，按鈕之後被 pack 進來時其實畫在視窗下緣以外，看不到。

新增 `_grow_window_if_needed()`：在「done」訊息處理完（成功／部分失敗／中止）
之後量一次目前實際需要的高度，不夠就往下補高，寬度與位置不動。
`resizable(False, False)` 只擋使用者手動拖曳，不擋程式自己改 `geometry()`，
不違反原本固定視窗大小的設計。124 個測試全過。

### 2026-08-26 — API Key 改為明確「儲存」按鈕

原本「記住」勾選框要等按下「開始轉錄」才會把 Key 寫進 `.env`，使用者換了 Key
之後在按開始之前不知道有沒有存到。拿掉「記住」勾選框，改成 API Key 輸入框旁一顆
「儲存」按鈕，按下立即寫入 `.env`，並在旁邊顯示「✓ 已儲存」（3 秒後自動消失）。
存 Key 與開始轉錄拆成兩件事，`_start()` 不再附帶儲存邏輯。四語（繁中／简中／
英文／日文）介面文字同步補齊 `gui.btn.save_key`、`gui.lbl.api_saved`。
