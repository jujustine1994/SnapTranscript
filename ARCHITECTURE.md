...
Run SnapTranscript.bat
  └─ launcher.ps1
        ...
        └─ tkinter 視窗啟動
              ...
              │     └─ 自動模式：尾巴不足 N 分鐘併回前一段（UI 可調，0 = 關閉；
              │                   自訂切割點模式下此欄位 disable）
              ...
              └─ 關閉視窗
                    ├─ 任務進行中 → 跳確認對話框，按「否」則不關
                    └─ 確定關閉 → 清掉本行程 PID 的暫存檔後 destroy
```
...
> **判讀 app.log 的陷阱：** 2026-07-31 以前的測試沒有 patch `logger.write_log`，
> 在真實的 `app.log` 裡留下約 1,970 行假錯誤。這些殘留的特徵是**同一秒內出現
> 幾十上百行**（測試的 sleep 是假的，整批在同一秒寫完）、而且**沒有對應的
> `=== 任務起始 ===` 標頭**。統計錯誤率時要先濾掉，否則會把 2,064 行誤判成
> 真實故障——實際上真實使用的錯誤行只有 92 行。濾法：
> `grep ERROR logs/app.log | cut -c1-11 | uniq -c | awk '$1<10'`。
> 現在的測試已用 `JobTestBase` patch 掉 `write_log`，跑完整套測試不會再新增任何一行
> （實測前後皆 2,103 行）。
...
重試中的錯誤行末尾會標示進度，**兩種模式寫法不同**（2026-08-02 修正）：

| 模式 | 格式 | 說明 |
|------|------|------|
| 勾選自動重試 | `重試 3/5` | 有上限，數字對應 `MAX_AUTO_RETRIES` |
| 未勾選（手動） | `手動重試 3` | 無上限，只記已重試次數 |

原本兩種模式共用 `重試 N/5` 的寫法，但計數器只在自動分支遞增——手動模式
每一行都是 `重試 0/5`，使用者按了 1 次還是 20 次「是」，log 長得一模一樣，
事後完全查不出來，而且 `/5` 還暗示了一個手動模式根本沒有的上限。
...
| `MIN_SEGMENT_SECONDS` | 300（5分） | 自動切割時，短於此秒數的尾巴段落併回前一段（設 0 關閉）。這只是 UI 欄位的**預設值**，使用者可在視窗中改 |

### 短尾巴段落為何要合併

自動模式每 30 分鐘一刀，2 小時 33 分的音訊會切出最後一段只有 3 分鐘——要付一次
完整的切割 + 上傳 + API 呼叫，換來幾乎沒有內容的逐字稿，還在輸出檔裡多一個
段落標題。所以 `plan_segments()` 在自動模式下會把不足 `MIN_SEGMENT_SECONDS`
的尾巴撤掉最後一刀，併回前一段。

**自訂切割點不套用這個合併**：使用者手動輸入的位置是明確意圖，程式不該擅自改動。
UI 上切到「自訂切割點」時，門檻欄位會 disable。

#### 門檻上限：為何必須小於一段的長度

合併的實作是「撤掉最後一刀」，而且只撤一刀。門檻若 >= `DEFAULT_CHUNK_SECONDS`，
撤掉後的那一段會變成兩倍長（30 分的設定會切出 60 分的段落），輸出可能撞上
Gemini 的 65,536 token 上限**被靜默截斷——逐字稿少一截，卻沒有任何錯誤訊息**。
所以 `parse_min_segment_minutes()` 會擋下 >= 切割長度的輸入。

門檻設 5 分鐘時，最長的一段是 `DEFAULT_CHUNK_SECONDS + MIN_SEGMENT_SECONDS - 1`
...
- **關視窗時另外清一次**（`job.cleanup_temp_files()`）：背景執行緒是 daemon，
  行程一結束就被硬砍，`_process_one` 的 finally 不會執行。不清的話專案目錄
  會留下一個 27 MB 左右的殘檔，跑幾次就是好幾百 MB。清理只掃**自己 PID**
  的前綴，絕不動其他實例的檔案（理由同下）
